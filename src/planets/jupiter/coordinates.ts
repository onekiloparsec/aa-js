...
export function getEclipticLongitude (jd: JulianDay): Degree {
  ...
  if () {
    ...
    const L5 = (coeffs5 as PlanetCoefficient[]).reduce((sum, val) => sum.plus(val.A.mul(Math.cos(val.B.plus(val.C.mul(tau))))), new Decimal(0))
    value = (L0
        .plus(L1.mul(tau))
    ...
    const L5 = (coeffs5 as PlanetCoefficient[]).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * taunum)), 0)
    value = new Decimal(
...
export function getRadiusVector (jd: JulianDay): AstronomicalUnit {
  ...
  if () {
    ...
    const R5 = (coeffs5 as PlanetCoefficient[]).reduce((sum, val) => sum.plus(val.A.mul(Math.cos(val.B.plus(val.C.mul(tau))))), new Decimal(0))
    value = (R0
        .plus(R1.mul(tau))
        .plus(R2.mul(tau.pow(2)))
        .plus(R3.mul(tau.pow(3)))
        .plus(R4.mul(tau.pow(4)))
        .plus(R5.mul(tau.pow(5)))
    )
  } else {
    const taunum = tau.
    ...
    const R5 = (coeffs5 as PlanetCoefficient[]).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * taunum)), 0)
    value = new Decimal(
      R0 + R1 * taunum + R2 * Math.pow(taunum, 2) + R3 * Math.pow(taunum, 3) + R4 * Math.pow(taunum, 4) + R5 * Math.pow(taunum, 5)
    )
  }
