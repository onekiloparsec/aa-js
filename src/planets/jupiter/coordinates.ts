...
export function getRadiusVector (jd: JulianDay | number, highPrecision: boolean = true): AstronomicalUnit {
  ...
  const coeffs5 = getCoefficientsR5(highPrecision)
  ...
  if (highPrecision) {
    ...
    const R5 = (coeffs5 as PlanetCoefficient[]).reduce((sum, val) => sum.plus(val.A.mul(Decimal.cos(val.B.plus(val.C.mul(tau))))), new Decimal(0))
    value = (R0
        .plus(R1.mul(tau))
    ...
    const R5 = (coeffs5 as PlanetCoefficientNum[]).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * taunum)), 0)
    value = new Decimal(
