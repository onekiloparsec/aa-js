...
export function getMeanLongitude (jd: JulianDay): Degree {
  ...
  const T3 = T2 * T
  const T4 = T3 * T
  return fmod360(218.3164477 + 481267.88123421 * T - 0.0015786 * T2 + T3 / 538841 - T4 / 65194000)
}
...
export function getMeanElongation (jd: JulianDay): Degree {
  ...
  const T3 = T2 * T
  const T4 = T3 * T
  return fmod360(297.8501921 + 445267.1114034 * T - 0.0018819 * T2 + T3 / 545868 - T4 / 113065000)
}
...
export function getMeanAnomaly (jd: JulianDay): Degree {
  ...
  const T3 = T2 * T
  const T4 = T3 * T
  return fmod360(134.9633964 + 477198.8675055 * T + 0.0087414 * T2 + T3 / 69699 - T4 / 14712000)
}
...
export function getArgumentOfLatitude (jd: JulianDay): Degree {
  ...
  const T3 = T2 * T
  const T4 = T3 * T
  return fmod360(93.2720950 + 483202.0175233 * T - 0.0036539 * T2 - T3 / 3526000 + T4 / 863310000)
}
...
export function getEclipticLongitude (jd: JulianDay): Degree {
  ...
  return fmod360(LdashDegrees + SigmaL / 1000000 + NutationInLong / 3600)
}
...
export function getMeanLongitudeAscendingNode (jd: JulianDay): Degree {
  ...
  const T3 = T2 * T
  const T4 = T3 * T
  return fmod360(125.0445479 - 1934.1362891 * T + 0.0020754 * T2 + T3 / 467441 - T4 / 60616000)
}
...
export function getMeanLongitudePerigee (jd: JulianDay): Degree {
  ...
  const T3 = T2 * T
  const T4 = T3 * T
  return fmod360(83.3532465 + 4069.0137287 * T - 0.0103200 * T2 - T3 / 80053 + T4 / 18999000)
}
