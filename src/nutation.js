...
function getNutationInLongitude(JD) {
    ...
    for (let i = 0; i < g_NutationCoefficients.length; i++) {
        let argument = g_NutationCoefficients[i].D * D + g_NutationCoefficients[i].M * M +
            g_NutationCoefficients[i].Mprime * Mprime + g_NutationCoefficients[i].F * F +
            g_NutationCoefficients[i].omega * omega

        value += (g_NutationCoefficients[i].sincoeff1 + g_NutationCoefficients[i].sincoeff2 * T) *
...
function getMeanObliquityOfEcliptic(JD) {
    ...
    return sexagesimal.DMSToDegrees(23, 26, 21.448) - sexagesimal.DMSToDegrees(0, 0, 4680.93) * U
