...
describe('moon', () => {
  ...
  test('get moon mean longitude [low precision]', () => {
    expect(Earth.Moon.getMeanLongitude(245123456, false)).toBeCloseTo(182.1252505089, 6)
  })
  ...
  test('get moon mean elongation [low precision]', () => {
    expect(Earth.Moon.getMeanElongation(245123456, false)).toBeCloseTo(175.566305716, 6)
  })
  ...
  test('get moon mean anomaly [low precision]', () => {
    expect(Earth.Moon.getMeanAnomaly(245123456, false)).toBeCloseTo(343.8983220482618, 6)
  })
  ...
  test('get moon equatorial coordinates', () => {
    ...
    expect(equ.rightAscension).toBeCloseTo(getDecimalValue(8, 58, 45.12).hoursToDegrees()., 3)
  ...
  test('get moon equatorial coordinates [low precision]', () => {
    ...
    expect(Earth.Moon.getGeocentricEclipticLongitude(jd, false)).toBeCloseTo(133.162_655, 6)
    expect(Earth.Moon.getGeocentricEclipticLatitude(jd, false)).toBeCloseTo(-3.229_126, 6)
  ...
  test('get moon illumination fraction [low precision]', () => {
    ...
    expect(sunCoords.rightAscension).toBeCloseTo(20.76, 2)
    expect(sunCoords.declination).toBeCloseTo(8.737, 2)

    const moonCoords = Earth.Moon.getGeocentricEquatorialCoordinates(jd, false)
    expect(moonCoords.rightAscension).toBeCloseTo(134.69, 1)
    expect(moonCoords.declination).toBeCloseTo(13.7684, 2)

    expect(Earth.Moon.getRadiusVectorInKilometer(jd, false)).toBeCloseTo(368409.0, 1)
  ...
  test('get moon age', () => {
    ...
    expect(Earth.Moon.getAge(fqMoonJD)).toBeCloseTo(MOON_SYNODIC_PERIOD.dividedBy(4)., 6)
    const fullMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), MoonPhaseQuarter.Full)
    expect(Earth.Moon.getAge(fullMoonJD)).toBeCloseTo(MOON_SYNODIC_PERIOD.dividedBy(2)., 5)
    const lqMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), MoonPhaseQuarter.LastQuarter)
    expect(Earth.Moon.getAge(lqMoonJD)).toBeCloseTo(MOON_SYNODIC_PERIOD.mul(3).dividedBy(4)., 5)
  })
